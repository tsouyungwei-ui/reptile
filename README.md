...
本專案完全可以由開發者自行在電腦的終端機 (Terminal) 中獨立運作，不需要依賴 AI 代為操作。
每次開啟新的終端機視窗，準備執行爬蟲前，請務必先進入專案資料夾並啟動 Python 虛擬環境：

```bash
cd <你的專案資料夾路徑>   # 例如: cd ~/Desktop/reptile
source venv/bin/activate
```

啟動環境後，即可直接在終端機輸入下方的「單機執行」或「多台電腦分散執行」指令來啟動爬蟲。
...
本爬蟲具備**嚴謹的防呆與自動重試機制**：
- **實體檔案驗證**：每次執行時，程式不僅會檢查資料庫的紀錄，還會確認該季度的 PDF 是否「真實存在」且「大小正常 (>50KB)」。如果實體檔案因故遺失或損壞，程式會自動重新下載。
- **補齊缺漏資料**：若過去因為網路不穩、伺服器暫時阻擋，或是提早執行而查無資料，只要本地端沒有該季度的 PDF，下次執行爬蟲時便會**自動重新嘗試連線尋找**，確保「只要沒抓到，下次一定重抓」。
- **精準範圍**：程式會依據「上市年份」智慧判斷搜尋起點，不會浪費時間在公司上市前根本沒有財報的年份。
